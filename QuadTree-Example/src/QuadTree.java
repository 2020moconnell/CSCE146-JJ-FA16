...
public class QuadTree 
{
	private class Node
	{
		...
		private double x,y;//top left point of box
	...
	public QuadTree(double aSideLength, int aDepth)
	{
		if(aDepth < 1 && aSideLength <= 0)
			return;
		// need to subdivide
		root = subdivideNode(0,0,aSideLength, aDepth);
	}
	private Node subdivideNode(double aX, double aY, double aSideLength, int aDepth)
	{
		Node aNode = new Node(aX, aY, aSideLength);
		if(aDepth < 0)
		{
			double halfSide = aSideLength / 2.0;
	...
	{
		return aNode != null && aNode.x < aCollider.getX() && aCollider.getX() < aNode.x + aNode.sideLength && 
